...
if($REQUEST_METHOD == "POST") {
    ...
    if(isset($userInfo['address_id'])) {
        $address_id = trim($userInfo['address_id']);
    }
    ...
    if($userFound != false) {            
            ...
            if($userFound['is_active'] == '0') {
                $result= ([
                    "data" => "Address is already suspended",
                    "title" => "",
                    "status" => "fail",
                    "statusCode" => 202
                ]);
            ...
            if($userFound['is_active'] == '1') {
                ...
                $sql_update = "UPDATE user_vs_address 
                SET address_type = ?, 
                    address_line_1 = ?, 
                    address_line_2 = ?, 
                    taluka = ?, 
                    ...
                    pincode = ?, 
                    business_name = ? 
                WHERE user_id = ? AND id_user_vs_address = ?";

                $stmt_update = $pdo->prepare($sql_update);
                $stmt_update->execute([$address_type, $address_line_1, $address_line_2, $taluka, $city, $state, $pincode, $business_name, $user_id, $address_id]);


                $result= ([
                    "data" => "Address is Updated successfuly.",
                    "title" => "",
                    "status" => "success",
                    "statusCode" => 200
                ]); 
