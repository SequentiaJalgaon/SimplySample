...
if ($REQUEST_METHOD == "PUT") {
    ...
    // Required Fields
    $requiredFields = ['user_id', 'order_id', 'product_id', 'rating', 'message', 'review_id'];
    ...
    $order_id = trim($userInfo['order_id']);
    $product_ids = $userInfo['product_id']; // Array
    $ratings = $userInfo['rating']; // Array
    $messages = $userInfo['message']; // Array
    $review_ids = $userInfo['review_id']; // Array

    // Validate Arrays Length
    if (count($product_ids) !== count($ratings) || count($product_ids) !== count($messages) || count($product_ids) !== count($review_ids)) {
    ...
    foreach ($messages as $message) {
        if (strlen($message) < 100 || strlen($message) > 500) {
            echo json_encode(["data" => "Each message must be between 100 and 500 characters.", "status" => "fail", "statusCode" => 400]);
            exit();
        }
    }
    ...
    try {
        ...
        for ($i = 0; $i < count($product_ids); $i++) {
            ...
            $sqlCheckReview = "SELECT * FROM rating_review WHERE id = ? AND user_id = ? AND order_id = ? AND product_id = ?";
            $qCheckReview = $pdo->prepare($sqlCheckReview);
            $qCheckReview->execute([$review_id, $user_id, $order_id, $product_id]);
            if (!$qCheckReview->fetch(PDO::FETCH_ASSOC)) {
                echo json_encode(["data" => "Review ID $review_id does not exist or does not belong to the user", "status" => "fail", "statusCode" => 404]);
                exit();
            }

            // Update review in the database
            $sqlUpdate = "UPDATE rating_review SET rating = ?, message = ? WHERE id = ?";
            $stmtUpdate = $pdo->prepare($sqlUpdate);
            $stmtUpdate->execute([$rating, $message, $review_id]);
        ...
        echo json_encode([
            "data" => "$updatedReviews reviews updated successfully.",
            "status" => "success",
            "statusCode" => 200
        ]);
    } catch (Exception $e) {
        ...
        $pdo->rollBack();
        echo json_encode(["data" => "Error updating reviews: " . $e->getMessage(), "status" => "fail", "statusCode" => 500]);
    }
